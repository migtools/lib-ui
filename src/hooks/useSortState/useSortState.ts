...
export const useSortState = <T>(
  ...
  const onSort = (_event: React.SyntheticEvent, index: number, direction: SortByDirection) => {
  ...
  return {
    ...
    onSort,
    sortedItems,
    setSortColumn: (index: number) => setSortBy({ ...sortBy, index }),
    setSortDirection: (direction: SortByDirection) => setSortBy({ ...sortBy, direction }),
  };
};
